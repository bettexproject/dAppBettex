<template>
  <div :class="stateClass" class="bet-popup">
    <div class="bet-popup-info">
      <div class="bet-popup-title">{{title}}</div>
      <div class="bet-popup-bet">{{ value.side }} {{ betType }}</div>
    </div>
    <div class="bet-popup-form">
      ...
      <div class="bet-popup-form-group" :class="isAmountValid ? 'valid' : 'invalid'">
        <label for="amount" class="bet-popup-label">
          Amount
          <span class="asset-name">USD</span>
        </label>
        <input
          class="bet-popup-input"
          type="text"
          id="amount"
          style="width: 80px;"
          v-model="amount"
    ...
    <div class="bet-popup-makebet">
      <button
        @click.prevent="doBet"
        class="bet-popup-button"
        :disabled="this.lock || !isValid"
        :class="{'bet-popup-button-for': value.side==='for'}"
      >
        Place
        bet
      </button>
      <span v-if="value.side === 'for'" class="bet-popup-profit"> Profit {{ profit }} <span class="asset-name">USD</span></span>
      <span v-else class="bet-popup-profit"> Escrow {{ escrow }} <span class="asset-name">USD</span></span>
    </div>
...
export default {
  ...
  computed: {
    ...
    escrow() {
      const p = (this.amount * (this.odds - 1)).toFixed(2);
      return isNaN(p) ? "-" : p;
    },
    stateClass() {
    ...
    betType() {
      return (
        this.value && this.value.subevent && subeventName(this.value.subevent)
      );
    },
    isOddsValid() {
      return (
        this.odds.toString() === parseFloat(this.odds).toString() &&
        this.odds > 1
      );
    },
    isAmountValid() {
      return (
        this.amount.toString() === parseFloat(this.amount).toString() &&
